...
public interface WorkerRepository extends JpaRepository<Worker, Long> {
    ...
    @Query("select new uz.zako.oquv_markaz.payload.WorkerPayload(e.id,e.fullName,e.adress,p.id,e.monthly,cb.id) from Worker e inner join Phone p inner join CenterBranches cb where e.centerBranches.id=?1")
